    ...
    ILogger<AlertDispatcher> logger)
{
    ...
    {
        var isFiring = payload.Alerts.Any(a => a.Status == "firing");
        ...
        var details = payload.Alerts
            .Select(a =>
            {
                var labels = string.Join(", ", a.Labels.Select(kv => $"{kv.Key}={kv.Value}"));
        ...
        return new AlertNotification(
            ...
            Severity: severity,
            State: state,
            FiredAt: payload.Alerts.FirstOrDefault()?.StartsAt ?? DateTimeOffset.UtcNow
        );
    }
}
